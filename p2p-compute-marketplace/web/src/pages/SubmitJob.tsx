...
export default function SubmitJob() {
  ...
  const [result, setResult] = useState<{msg: string; status: 'success' | 'error'; id?: string; hash?: string} | null>(null)
            ...
            {result && (
              <div className={`mt-4 p-4 rounded-lg text-sm ${result.status === 'success' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'}`}>
                ...
                {result.status === 'success' && (
                  <Link to="/jobs" className="text-xs text-cyan-400 underline mt-2 inline-block">View in My Jobs</Link>
                )}
              </div>
            )}
          </div>

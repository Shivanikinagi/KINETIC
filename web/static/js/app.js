        ...
        <div class="group relative flex flex-col bg-surface-container-low rounded-xl border-r border-cyan-900/10 hover:bg-surface-container transition-all duration-500 overflow-hidden">
            ...
            <div class="p-8 space-y-6">
                ...
                <div class="pt-6 border-t border-white/5 flex justify-between items-center">
                    ...
                    <button 
                        class="bg-surface-container-highest hover:bg-cyan-400 hover:text-on-primary px-6 py-2 rounded-lg font-bold transition-all ${provider.status !== 'active' ? 'opacity-50 cursor-not-allowed' : ''}"
                        ${provider.status !== 'active' ? 'disabled' : ''}
                        onclick="provisionProvider('${provider.id}')"
                    >
...
function provisionProvider(providerId) {
    if (!walletConnected) {
    ...
    alert(`Provisioning ${providerId}. This will be integrated with smart contracts.`);
